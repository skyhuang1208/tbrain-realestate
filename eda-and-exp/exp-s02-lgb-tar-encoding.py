...
cols_feats = cols_num + ['encoded_' + c for c in cols_cat] + ['encoded_' + c for c in cols_bin]
...
class FeatureEngineering():
    def __init__(self, cols_cat, cols_bin):
        self.encoder = None
        self.cols_catbin = cols_cat + cols_bin

    def fit_transform(self, df):
        # Target-mean encoding
        self.encoder = TargetMeanEncoding(col_target = 'target')
        encoded = self.encoder.fit_transform(df[self.cols_catbin], df['target'])
        return pd.concat([df, encoded], axis=1)

    def transform(self, df):
        encoded = self.encoder.transform(df[self.cols_catbin])
        return pd.concat([df, encoded], axis=1)
...
for i_fold, (itrain, ival) in enumerate(folds.split(df)): # kfold
    ...
    # feat eng
    feat_eng = FeatureEngineering(cols_cat, cols_bin)
